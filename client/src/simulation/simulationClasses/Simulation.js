...
export default class Simulation {
    ...
    */
    ...
    setPreviousAnimationTime() {
        this.setNextAnimationTimeToSpecificTimepoint(
            this.animationTime - this.simCtx.animationTimeInterval
        );

        if (this.onAnimationTimeCallback.length) {
            this.onAnimationTimeCallback.forEach((callback) =>
                callback(this.getSimInfo())
            );
        }
    }
    ...
    setNextAnimationTimeToSpecificTimepoint(timepoint) {
        ...
        if (timepoint % this.simCtx.animationTimeInterval > 0) {
            timepoint +=
                ...
                (timepoint % this.simCtx.animationTimeInterval);
        }
    ...
    async addShip(shipInfo, isCaseShip = false) {
        const paramsPassingShip = isCaseShip
            ...
            : {
                  ...
                  posY:
                      this.caseShip.width / 2 +
                      shipInfo.deltaYShips +
                      shipInfo.width / 2,
        ...
        if (isCaseShip) {
            ...
            if (this.caseShip) {
                ...
                console.log(
                    "!!! ERROR: Make sure to firstly set the caseShip before any other ship as this ship defines the origin of the simulation!"
                );
            }
        }
    }
    ...
    async addHawsers(bolderData, hawserLimits, hawserBreakingTimePoints = []) {
        return new Promise((resolve, reject) => {
            ...
            hawserBreakingTimePoints.forEach((hawserBreakingTimePoint) => {
                this.hawserArray[
                    hawserBreakingTimePoint.hawserId
                ].setBreakingTimePoint(hawserBreakingTimePoint.timePoint);
            });
            ...
            this.hawserArray.forEach(async (hawser) => {
                ...
                if (
                    this.hawserArray[this.hawserArray.length - 1].imageIsLoaded
                ) {
                    resolve();
                }
            });
    ...
    */
    ...
    fpsIntervalIsElapsed() {
        ...
        if (
            this.fpsIntervalInMilS <
            this.currentFrameTimeStamp - this.lastFrameTimeStamp
        ) {
            this.frameCount += 1;
        }
    ...
    */
    ...
    updateSimulation() {
        ...
        this.caseShip.setPosX(
            timePoint.shipData.posX * this.translationAmplifierFactor
        );
        this.caseShip.setPosY(
            timePoint.shipData.posY * this.translationAmplifierFactor
        );
        this.caseShip.rotationInDegrees =
            timePoint.shipData.rotation * this.translationAmplifierFactor;

        // update passingShip parameters
        this.passingShips.forEach((passingShip) => {
            passingShip.applySpeedDisplacement(
                this.animationTime * this.timePointInterval
            );
        });

        // update hawsers parameters
        this.hawserArray.forEach((hawser, index) => {
            hawser.setPosOnShipX(
                timePoint.hawserData[index].posXShip,
                this.translationAmplifierFactor
            );
            hawser.setPosOnShipY(
                timePoint.hawserData[index].posYShip,
                this.translationAmplifierFactor
            );
