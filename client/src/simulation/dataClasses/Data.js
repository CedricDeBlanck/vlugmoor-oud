...
export default class Data {
    ...
    async addTimePoints(dataCoords, dataForces, shipTranslation) {
        return new Promise((resolve, reject) => {
            ...
            for (let time = 0; time < dataCoords.length - 1; time++) {
                ...
                for (
                    let hawser = 0;
                    hawser < this.caseMetaData.bolderData.length;
                    hawser++
                ) {
                    ...
                    const hawserData = new HawserData(
                        hawser,
                        Number(
                            dataCoords[time][columnNrInCoordsTable].replace(
                                ",",
                                "."
                            )
                        ),
                        Number(
                            dataCoords[time][columnNrInCoordsTable + 1].replace(
                                ",",
                                "."
                            )
                        ),
                ...
                for (
                    let fenderNr = 0;
                    fenderNr < this.caseMetaData.fenderData.length;
                    fenderNr++
                ) {
                    // In de table zijn er eerst de forces op de trossen  en dan 3 translatie kolommen
                    const columnNrInForcesTable =
                        this.caseMetaData.bolderData.length + 3 + fenderNr;
                    // make fenderData object
                    const fenderData = new FenderData(
                        Number(
                            dataForces[time][columnNrInForcesTable].replace(
                                ",",
                                "."
                            )
                        )
    ...
    registerBreakingTimePoint(hawserData, time) {
        ...
        if (
            this.checkIfHawserHasBroken(
                hawserData.loadRatio,
                this.caseMetaData.hawserLimits.first
            ) &&
            !this.checkIfHawserAlreadyHasBroken(hawserData.id)
        ) {
    ...
    checkIfHawserAlreadyHasBroken(id) {
        return (
            this.hawserBreakingTimePoints.filter(
                (hawserBreak) => hawserBreak.hawserId === id
            ).length > 0
        );
    }
